"""V9.1 模號 = v9 配方 + 現場 M28 圖（S1 外觀強健化診斷）。
  data = data_v92/mohao（data_v9 + 現場 M28 400 張）。其餘同 v9（yolov8s-cls、seed0、deterministic、不warm-start）。
...
def main():
    ...
    args = ap.parse_args()
    if not DATA.exists(): raise FileNotFoundError("data_v92/mohao missing（先跑 _build_v91_m28field.py）")
    overrides = dict(
        model=str(BASE), data=str(DATA), epochs=args.epochs,
        imgsz=args.imgsz, batch=args.batch, device=args.device, workers=args.workers,
        patience=args.patience, project=str(HERE/"runs_v92"), name="mohao", exist_ok=True,
    ...
    print(f"[V9.2 mohao] v9.1+M28-04現場（seed={args.seed} deterministic）")
