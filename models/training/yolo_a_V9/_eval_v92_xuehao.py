# -*- coding: utf-8 -*-
"""V9.2 穴號評估：v6.7.3 xuehao vs v9.2 xuehao
  (1) data_v671/xuehao/val 每穴號零退步
  (2) _m28_04_holdout（200 未見 M28-04 raw）→ 穴號是否判成 04
Run: python _eval_v92_xuehao.py --device 0
"""
...
WEIGHTS = {
 ...
 "v9.2":   HERE/"runs_v92"/"xuehao"/"weights"/"best.pt",
}
def top1(m,s,dev):
...
def main():
    ...
    cavs=sorted(R[tags[0]].keys())
    base="v6.7.3" if "v6.7.3" in tags else tags[0]
    print(f"\n===== data_v671/xuehao/val 每穴號（零退步 vs {base}）=====")
    hdr="穴號 "+"".join(f"{t:>10s}" for t in tags)+"  v9.2變化"; print(hdr); print("-"*len(hdr)); reg=[]
    for c in cavs:
        row=f"{c:4s}"; accs={}
        for t in tags:
            ok,n=R[t][c]; acc=ok/n*100 if n else 0; accs[t]=acc; row+=f"  {acc:6.1f}"
        if base in tags and "v9.2" in tags:
            d=accs["v9.2"]-accs[base]; row+=(f"  ↓{-d:.1f}" if d<-1e-6 else (f"  ↑{d:.1f}" if d>1e-6 else "  ="))
            if d<-1e-6: reg.append((c,d))
        print(row)
    print("\n===== _m28_04_holdout（200 未見 M28-04）→ 穴號判成 04 比例 =====")
    for t in tags:
        ok,n,miss,wr=eval_hold(YOLO(str(WEIGHTS[t])),a.device)
        print(f"  [{t}] 04 {ok}/{n}={ok/n*100 if n else 0:.1f}%  漏{miss}  誤判={wr}")
    print(("\n⚠ v9.2 穴號退步: "+str([(c,round(d,1)) for c,d in reg])) if reg else f"\n✅ v9.2 穴號對每穴 ≥ {base}")
