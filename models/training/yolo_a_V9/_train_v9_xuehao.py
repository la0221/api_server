...
def main():
    ...
    overrides = dict(
        ...
        patience=args.patience, project=str(HERE/"runs"), name="xuehao", exist_ok=True,
    ...
    print(f"[V9 xuehao] 全量從頭重訓 yolov8s-cls（穴號 01–18；data_v671 + data3/M17；seed={args.seed} deterministic）")
    trainer = make_trainer()(overrides=overrides)
    trainer.train()
    print(f"[V9 xuehao] best -> {trainer.best}")
