...
public sealed class SqliteWorkOrderRepository : IWorkOrderRepository
{
    ...
    {
        const string sql = @"
            SELECT Id, Code, ProductName, ModelName, MachineModelName, StartAt, EndAt, Status
            FROM WorkOrders
            WHERE Code = @Code
            LIMIT 1;
        ";
    ...
    {
        const string sql = @"
            INSERT INTO WorkOrders (Id, Code, ProductName, ModelName, MachineModelName, StartAt, EndAt, Status, CreatedAt, UpdatedAt)
            VALUES (@Id, @Code, @ProductName, @ModelName, @MachineModelName, @StartAt, @EndAt, @Status, @CreatedAt, @UpdatedAt);
        ";
