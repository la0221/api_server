...
public partial class App : System.Windows.Application
{
    ...
    private void ConfigureServices(HostBuilderContext context, IServiceCollection services)
    {
        ...
        // 根據 AiSettings.Mode 動態選擇推論實作（圖片點檢 AI 推論）
        services.AddTransient<AIVision.Application.Ports.ImageBatch.IAiInferencePort>(sp =>
        {
            ...
            return string.Equals(cfg.Mode, "segmentation", StringComparison.OrdinalIgnoreCase)
                ? (AIVision.Application.Ports.ImageBatch.IAiInferencePort)sp.GetRequiredService<HttpSegmentationInferencePort>()
                : sp.GetRequiredService<HttpClassificationInferencePort>();
        });
        ...
        // 註冊批量推論 ViewModel（使用專用的推論 Port）
        services.AddTransient<BatchInferenceViewModel>(sp =>
        {
            var folderPicker = sp.GetRequiredService<IFolderPickerPort>();
            var enumerator = sp.GetRequiredService<IImageEnumeratorPort>();
            var loader = sp.GetRequiredService<IImageLoaderPort>();
            var ai = sp.GetRequiredService<HttpBatchInferencePort>();
            var writer = sp.GetRequiredService<IImageWriterPort>();
            var messenger = sp.GetRequiredService<IMessenger>();
            var workOrderService = sp.GetRequiredService<IWorkOrderManagementService>();
            var inspectionRepository = sp.GetRequiredService<IInspectionRepository>();
            var navigationService = sp.GetRequiredService<INavigationService>();
            var modelConfigService = sp.GetRequiredService<ModelConfigService>();
            var logger = sp.GetService<ILogger<BatchInferenceViewModel>>();
            return new BatchInferenceViewModel(folderPicker, enumerator, loader, (AIVision.Application.Ports.ImageBatch.IAiInferencePort)ai, writer, messenger, workOrderService, inspectionRepository, navigationService, modelConfigService, logger);
        });
        ...
        switch (cameraType.ToLowerInvariant())
        {
            ...
            case "idspeak":
                ...
                // 註冊 Line Scan 服務（需要 IdsCameraPort）
                services.AddSingleton<ILineScanService, LineScanService>();
                // 註冊 Line Scan 模擬器
                services.AddSingleton<ILineScanSimulator, LineScanSimulator>();
                // 註冊 Auto Run 服務（Line Scan 專用）
                services.AddAutoRunService();
                break;
        }
        ...
        // 註冊模型發現服務
        services.AddSingleton<IModelDiscoveryService, LocalModelDiscoveryService>();
        ...
        services.AddSingleton<ModelConfigService>(sp =>
        {
            ...
            if (options.AutoScan)
            {
                logger.LogInformation("[DI] 註冊 LocalModelDiscoveryService - ScanFolder: {Folder}", options.ScanFolder);
            }
        ...
        // 註冊 SwitchableAiInferencePort（支援運行時動態切換單一模型/Workflow）
        services.AddSingleton<SwitchableAiInferencePort>();

        // 使用 SwitchableAiInferencePort 作為 IAiInferencePort 的實作
        services.AddSingleton<AIVision.Application.Ports.Devices.IAiInferencePort>(sp =>
        {
            var logger = sp.GetRequiredService<ILogger<App>>();
            var switchablePort = sp.GetRequiredService<SwitchableAiInferencePort>();

            // 根據配置決定預設模式
            if (workflowEnabled)
            {
                logger.LogInformation("[DI] 預設使用 Workflow 模式推論（可動態切換）");
                switchablePort.SwitchTo(AIVision.Application.Configuration.InferenceType.Workflow);
            }
            else
            {
                logger.LogInformation("[DI] 預設使用單一模型模式推論（可動態切換）");
                switchablePort.SwitchTo(AIVision.Application.Configuration.InferenceType.SingleModel);
            }

            return switchablePort;
        });
        ...
        services.AddTransient<LineScanViewModel>();
        ...
        services.AddTransient<LineScanView>();
    }
