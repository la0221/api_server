...
public partial class ShellViewModel : ObservableObject, IDisposable
{
    ...
    // 模號核對運轉服務（面掃站；用於把當前工單的預期模號與工單 Id 帶入辨識週期 + 持久化）
    private readonly AIVision.Application.MoldCode.AreaRunService? _areaRunService;

    // 模號模型抽換（讓當前辨識器跟隨工單選定的本地 ONNX 模型；prefix 取自 MoldCodeOnnx 設定）
    ...
    public ShellViewModel(
        ...
        AIVision.Application.MoldCode.AreaRunService? areaRunService = null,
        IMoldCodeModelSwitch? modelSwitch = null,
        IOptions<MoldCodeOnnxOptions>? moldCodeOnnxOptions = null)
    {
        _sender = sender;
        ...
        _areaRunService = areaRunService;
        _modelSwitch = modelSwitch;
        _moldCodePrefix = moldCodeOnnxOptions?.Value?.CodePrefix ?? "M101";
    ...
    {
        _navigationService.ShowWindow<OfflineModelManagementView>();
    }

    [RelayCommand]
    private void OpenOnlineModelManagement()
    {
        _navigationService.ShowWindow<OnlineModelManagementView>();
    }
    ...
    [RelayCommand]
    private void OpenMoldCodeBatch() => _navigationService.ShowWindow<MoldCodeBatchView>();

    [RelayCommand]
    private void OpenMoldCodePairBatch() => _navigationService.ShowWindow<MoldCodePairBatchView>();
    ...
    /// <summary>系統選單「API 伺服器設定」：選擇/測試中央推論 server（執行期生效）。</summary>
    [RelayCommand]
    ...
    private async Task StartPlcModeAsync(bool runOnce)
    {
        try
        {
            ...
            _logger?.LogInformation("========== 開始啟動 PLC {Mode} (Area Scan) ==========", modeText);
    ...
    private async Task StopPlcModeAsync()
    {
        try
        {
            _logger?.LogInformation("========== 停止 PLC 模式 (Area Scan) ==========");
    ...
    private async Task LoadCurrentWorkOrderAsync()
    {
        try
        {
            ...
            if (workOrder != null)
            {
                ...
                // 將當前工單的預期模號與 Id 帶入模號核對運轉服務（供辨識週期核對 + 持久化）
                if (_areaRunService != null)
                {
                    _areaRunService.ExpectedCode = workOrder.ExpectedMoldCode;
                    _areaRunService.WorkOrderId = workOrder.Id;
                }

                // 讓當前辨識器跟隨工單選定的本地 ONNX 模型（失敗不阻斷工單載入）
                await SwitchModelForWorkOrderAsync(workOrder.ModelName);
            ...
            else
            {
                ...
                // 清除模號核對運轉服務的預期模號與工單 Id
                if (_areaRunService != null)
                {
                    _areaRunService.ExpectedCode = null;
                    _areaRunService.WorkOrderId = null;
                }
    ...
    /// <summary>
    /// 依工單選定的模型名稱抽換當前模號辨識器(僅 .onnx 本地模型)。
    /// 找不到模型或非 .onnx 一律略過;任何錯誤只記錄,不可阻斷工單載入。
    /// </summary>
    private async Task SwitchModelForWorkOrderAsync(string? modelName)
    {
        if (_modelSwitch == null || string.IsNullOrWhiteSpace(modelName))
            return;
        ...
        try
        {
            ...
            {
                _modelSwitch.LoadModel(model.ModelPath, model.ResultTypes, _moldCodePrefix);
                _logger?.LogInformation("已依工單模型切換模號辨識器: {ModelName} -> {Path}", modelName, model.ModelPath);
            }
        }
        catch (Exception ex)
        {
            _logger?.LogWarning(ex, "依工單模型切換模號辨識器失敗: {ModelName}", modelName);
        }
    }
    ...
    public async Task CleanupAsync()
    {
        ...
        try
        {
            ...
            if (_plcHandshake.IsRunning)
            {
                _logger?.LogInformation("[Cleanup 1/5] 停止 PLC 交握服務...");
                try
                {
                    await WithTimeout(_plcHandshake.StopAsync(), 5000, "PlcHandshake.StopAsync");
                    _logger?.LogInformation("[Cleanup 1/5] ✓ PLC 交握服務已停止 ({ElapsedMs}ms)", sw.ElapsedMilliseconds);
                }
                catch (Exception ex)
                {
                    _logger?.LogWarning(ex, "[Cleanup 1/5] 停止 PLC 交握服務時發生錯誤");
                }
            }
            else
            {
                _logger?.LogInformation("[Cleanup 1/5] PLC 交握服務未運行，跳過");
            }

            // 2. 斷開 PLC 連線
            if (_plcCommunication.IsConnected)
            {
                _logger?.LogInformation("[Cleanup 2/5] 斷開 PLC 連線...");
                try
                {
                    await WithTimeout(_plcCommunication.DisconnectAsync(), 3000, "PlcCommunication.DisconnectAsync");
                    _logger?.LogInformation("[Cleanup 2/5] ✓ PLC 連線已斷開 ({ElapsedMs}ms)", sw.ElapsedMilliseconds);
                }
                catch (Exception ex)
                {
                    _logger?.LogWarning(ex, "[Cleanup 2/5] 斷開 PLC 連線時發生錯誤");
                }
            }
            else
            {
                _logger?.LogInformation("[Cleanup 2/5] PLC 未連線，跳過");
            }

            // 3. 停止 IoPanel 輪詢
            _logger?.LogInformation("[Cleanup 3/5] 停止 IoPanel 輪詢...");
            _ioPanelViewModel?.StopPolling();
            _logger?.LogInformation("[Cleanup 3/5] ✓ IoPanel 輪詢已停止 ({ElapsedMs}ms)", sw.ElapsedMilliseconds);

            // 4. 停止相機預覽並關閉相機
            if (_camera.IsOpen)
            {
                _logger?.LogInformation("[Cleanup 4/5] 關閉相機...");
                try
                {
                    ...
                    _logger?.LogInformation("[Cleanup 4/5] ✓ 相機已關閉 ({ElapsedMs}ms)", sw.ElapsedMilliseconds);
                }
                catch (Exception ex)
                {
                    _logger?.LogWarning(ex, "[Cleanup 4/5] 關閉相機時發生錯誤");
                }
            }
            else
            {
                _logger?.LogInformation("[Cleanup 4/5] 相機未開啟，跳過");
            }

            // 5. 停止光源控制器（釋放 TCP Server 監聽資源）
            if (_lightPort is IAsyncDisposable disposableLight)
            {
                _logger?.LogInformation("[Cleanup 5/5] 停止光源控制器...");
                try
                {
                    await WithTimeout(disposableLight.DisposeAsync().AsTask(), 3000, "LightPort.DisposeAsync");
                    _logger?.LogInformation("[Cleanup 5/5] ✓ 光源控制器已停止 ({ElapsedMs}ms)", sw.ElapsedMilliseconds);
                }
                catch (Exception ex)
                {
                    _logger?.LogWarning(ex, "[Cleanup 5/5] 停止光源控制器時發生錯誤");
                }
            }
            else
            {
                _logger?.LogInformation("[Cleanup 5/5] 光源控制器未初始化，跳過");
            }
