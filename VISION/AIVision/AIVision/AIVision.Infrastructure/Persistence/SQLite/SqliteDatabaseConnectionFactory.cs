...
public sealed class SqliteDatabaseConnectionFactory : IDatabaseConnectionFactory
{
    ...
    private static async Task ExecuteSchemaAsync(IDbConnection connection, CancellationToken cancellationToken)
    {
        ...
        // 數據庫遷移（additive，皆走 pragma_table_info 檢查避免重複 ALTER）：
        // - WorkOrders.MachineModelName（既有）
        // - WorkOrders.ExpectedMoldCode（PHASE B：操作員預期模號）
        // - Inspections.{ExpectedCode,ReadCode,Outcome,AirBlown}（PHASE C：模號三態核對寫入路徑）
        await EnsureColumnExistsAsync(connection, "WorkOrders", "MachineModelName", "TEXT", cancellationToken);
        await EnsureColumnExistsAsync(connection, "WorkOrders", "ExpectedMoldCode", "TEXT", cancellationToken);
        await EnsureColumnExistsAsync(connection, "Inspections", "ExpectedCode", "TEXT", cancellationToken);
        await EnsureColumnExistsAsync(connection, "Inspections", "ReadCode", "TEXT", cancellationToken);
        await EnsureColumnExistsAsync(connection, "Inspections", "Outcome", "TEXT", cancellationToken);
        await EnsureColumnExistsAsync(connection, "Inspections", "AirBlown", "INTEGER", cancellationToken);
    }

    /// <summary>
    /// 加性遷移：若指定資料表缺少該欄位則 ALTER TABLE 新增（mirror MachineModelName 的 pragma_table_info 檢查）。
    /// </summary>
        ...
        CancellationToken cancellationToken)
    {
        ...
        // 表名/欄名來自呼叫端常數（非使用者輸入），不可參數化 pragma 函式，故以字面字串組裝。
