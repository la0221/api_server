...
public sealed class WorkOrderManagementService : IWorkOrderManagementService
{
    ...
    public async Task<WorkOrder?> GetCurrentWorkOrderAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            // 首次存取且記憶體無工單 → 從 DB 還原「最近一筆仍進行中(Active)」的工單，
            // 讓 App 重啟後目前工單不會消失（只還原一次，之後尊重 End/Switch 的明確操作）。
            if (_currentWorkOrder == null && !_restoredFromDb)
            {
                _restoredFromDb = true;
                try
                {
                    var all = await _repository.GetAllAsync(cancellationToken);
                    _currentWorkOrder = all
                        .Where(w => w.Status == "Active" && !w.IsClosed)
                        .OrderByDescending(w => w.StartAt)
                        .FirstOrDefault();
                    if (_currentWorkOrder != null)
                        _logger.LogInformation("[WorkOrder] 啟動還原目前工單: {Code}", _currentWorkOrder.Code);
                }
                catch (Exception ex)
                {
                    _logger.LogWarning(ex, "[WorkOrder] 啟動還原目前工單失敗（忽略）");
                }
            }
    ...
    {
        return await CreateWorkOrderAsync(productName, modelName, machineModelName, null, null, cancellationToken);
    }
    ...
    public async Task<WorkOrder> CreateWorkOrderAsync(
        ...
        CancellationToken cancellationToken)
    {
        return await CreateWorkOrderAsync(productName, modelName, machineModelName, customWorkOrderCode, null, cancellationToken);
    }
    ...
    public async Task<WorkOrder> CreateWorkOrderAsync(
        ...
        CancellationToken cancellationToken)
    {
        _logger.LogDebug("開始建立工單: Product={ProductName}, Model={ModelName}, CustomCode={CustomCode}, ExpectedMoldCode={Expected}",
            productName, modelName, customWorkOrderCode, expectedMoldCode);
        ...
        try
        {
            ...
            var workOrder = new WorkOrder(
                ...
                expectedMoldCode: string.IsNullOrWhiteSpace(expectedMoldCode) ? null : expectedMoldCode.Trim());
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            var workOrder = await _repository.GetByIdAsync(workOrderId, cancellationToken)
                ?? throw new InvalidOperationException($"工單 {workOrderId} 不存在");

            workOrder.UpdateDetails(productName, machineModelName, expectedMoldCode);
            await _repository.UpdateAsync(workOrder, cancellationToken);

            // 若編輯的是目前工單，同步記憶體中的參考。
            if (_currentWorkOrder != null && _currentWorkOrder.Id == workOrderId)
                _currentWorkOrder = workOrder;

            _logger.LogInformation("[WorkOrder] 已更新工單: {Code} (Product={Product}, Expected={Expected})",
                workOrder.Code, productName, expectedMoldCode);
        }
        finally
        {
            _lock.Release();
